...
<form name="mess" action="<?=$home?>/control/menuedit?id=<?=$id?>" method="post">
    <? temp::formid() /* ЭТА ФУНКЦИЯ ОБЯЗАТЕЛЬНА ДЛЯ ВСЕХ ФОРМ!!! */?>
    <div class="content">
        <div class="conthead">
            <h2><i class="icon-cog-alt"></i>Редактирование пункта меню</h2>
        </div>
        ...
        <div class="contbody_forms">
            ...
            <select name="link_sort">
                ...
                <option value="<?=$outs['id']?>" <?if($outs['sel']):?>selected="selected"<?endif?>><?=$outs['name']?></option>
                <?endforeach?>
                <option value="0" <?if($its_top):?>selected="selected"<?endif?>>- <?=lang('in_beginning')?></option>
            </select>
        </div>
        <?endif?>
        <div class="contbody_forms">
