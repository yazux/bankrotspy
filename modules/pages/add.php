...
if(isset($_POST['add_attachment']))
{
   ...
   if($file)
   {
     ...
     if($ext)
     {
       ...
       temp::assign('link', core::$home.'/pages/add?att='.$at_num);
     ...
     else
     {
       engine_head(lang('fileload'));
       temp::HTMassign('savevar', $save_data);
       temp::assign('link', core::$home.'/pages/add?att='.$att);
       temp::assign('name', lang('pr_article'));
...
elseif(isset($_POST['del_attachment']))
{
  ...
  if ($file_del)
  {
    $req_file = core::$db->query("SELECT * FROM `ds_pages_files` WHERE `id` = '".$file_del."' AND `attach` = '".$att."' LIMIT 1");
...
elseif(POST('submit'))
{
  ...
  if(!$error)
  {
    ...
    func::notify(lang('pr_adminpanel'), lang('stat_added'), core::$home.'/pages/'.$insert_id, lang('continue'));
  }  
}

engine_head(lang('new_page'));
$arr = array();
$out = array();
if($att)
{
  $at1 = core::$db->query('SELECT * FROM `ds_pages_files` WHERE `attach` = "'.$att.'" ORDER BY `id`;');
  if ($at1->num_rows)
  {
