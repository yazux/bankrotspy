...
$total = core::$db->query('SELECT COUNT(*) FROM `ds_article` LEFT JOIN `ds_art_comm_rdm` ON `ds_article`.`id` = `ds_art_comm_rdm`.`modid` AND `ds_art_comm_rdm`.`userid` = "' . core::$user_id . '" WHERE (`ds_art_comm_rdm`.`userid` IS NULL OR `ds_art_comm_rdm`.`rdmtime` < `ds_article`.`comtime`) AND `ds_article`.`type` = "0" AND `ds_article`.`comtime` > "'.(time() - (3 * 24 * 3600)).'";')->count();

$res = core::$db->query('SELECT `ds_article`.*, `ds_art_comm_rdm`.`rdmtime`, `ds_users`.`lastvisit`, `ds_users`.`avtime`, `ds_users`.`sex`, `ds_users`.`rights` FROM `ds_article` LEFT JOIN `ds_users` ON `ds_article`.`userid` = `ds_users`.`id` LEFT JOIN `ds_art_comm_rdm` ON `ds_article`.`id` = `ds_art_comm_rdm`.`modid` AND `ds_art_comm_rdm`.`userid` = "' . core::$user_id . '" WHERE (`ds_art_comm_rdm`.`userid` IS NULL OR `ds_art_comm_rdm`.`rdmtime` < `ds_article`.`comtime`) AND `ds_article`.`type` = "0" AND `ds_article`.`comtime` > "'.(time() - (3 * 24 * 3600)).'" ORDER BY `ds_article`.`comtime` DESC LIMIT '.nav::$start.', '.nav::$kmess.';');
...
while($data = $res->fetch_assoc())
{
  ...
  if($arr['comtime'] > 0)
    $sql .= 'SELECT COUNT(*) FROM `ds_comm` WHERE `module`="articles" AND `mid` = "'.$data['id'].'" AND  `time` > "'.$data['rdmtime'].'" AND `time` > '.(time() - (3 * 24 * 3600)).';';
...
engine_head(lang('new_comms'));
temp::HTMassign('out', $out);
if(CAN('stats_create', 0) OR CAN('add_stats_moderate', 0))
    temp::assign('can_cr_stat', 1);

temp::HTMassign('navigation', nav::display($total, core::$home.'/articles/newcomm?'));
temp::display('articles.newcomm');
engine_fin();