...
$total = core::$db->query('SELECT COUNT(*) FROM `ds_article` WHERE `ds_article`.`type` > "0" AND `userid` = "'.core::$user_id.'";')->count();

$res = core::$db->query('SELECT `ds_article`.*, `ds_users`.`lastvisit`, `ds_users`.`avtime`, `ds_users`.`sex`, `ds_users`.`rights` FROM `ds_article` LEFT JOIN `ds_users` ON `ds_article`.`userid` = `ds_users`.`id` WHERE `ds_article`.`type` > "0" AND `userid` = "'.core::$user_id.'" ORDER BY `ds_article`.`time` DESC LIMIT '.nav::$start.', '.nav::$kmess.';');
...
engine_head(lang('draft'));
temp::HTMassign('out', $out);
if(CAN('stats_create', 0) OR CAN('add_stats_moderate', 0))
  temp::assign('can_cr_stat', 1);

temp::HTMassign('navigation', nav::display($total, core::$home.'/articles/draft?'));
temp::display('articles.allarticles');
engine_fin();