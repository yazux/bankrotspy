...
$total = core::$db->query('SELECT COUNT(*) FROM `ds_article` WHERE `ds_article`.`type` = "0";')->count();
    
$res = core::$db->query('SELECT `ds_article`.*, `ds_users`.`lastvisit`, `ds_users`.`avtime`, `ds_users`.`sex`, `ds_users`.`rights` FROM `ds_article` LEFT JOIN `ds_users` ON `ds_article`.`userid` = `ds_users`.`id` WHERE `ds_article`.`type` = "0" ORDER BY `ds_article`.`time` DESC LIMIT '.nav::$start.', '.nav::$kmess.';');
