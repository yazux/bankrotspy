...
$user_id = intval(abs(GET('id')));
if(!$user_id)
  denied();

new nav; //Постраничная навигация

$total = core::$db->query('SELECT COUNT(*) FROM `ds_article` WHERE `userid` = "'.$user_id.'" AND `ds_article`.`type` = "0";')->count();
$res = core::$db->query('SELECT `ds_article`.*, `ds_users`.`lastvisit`, `ds_users`.`avtime`, `ds_users`.`sex`, `ds_users`.`rights` FROM `ds_article` LEFT JOIN `ds_users` ON `ds_article`.`userid` = `ds_users`.`id` WHERE `userid`= "'.$user_id.'" AND `ds_article`.`type` = "0" ORDER BY `ds_article`.`time` DESC LIMIT '.nav::$start.', '.nav::$kmess.';');
...
engine_head(html_entity_decode( (isset($arr['user']) ? $arr['user'] : lang('stats')), ENT_QUOTES, 'UTF-8'));
temp::HTMassign('out', $out);
if(CAN('stats_create', 0) OR CAN('add_stats_moderate', 0))
    temp::assign('can_cr_stat', 1);
if(isset($arr['user']))
{
  temp::assign('user_sh', $arr['user']);
  temp::assign('user_shid', $arr['userid']);
}
else
  temp::assign('user_sh', lang('stats'));
  
if(isset($arr['sex']))
  temp::assign('user_sex', $arr['sex']);
 

temp::HTMassign('navigation', nav::display($total, core::$home.'/articles/user?id='.$user_id.'&amp;'));
temp::display('articles.user');
engine_fin();
