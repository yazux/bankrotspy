...
if($id)
{
  ...
  if($res['id'])
  {
    if(isset($_POST['add_attachment']))
    {
      ...
      if($file)
      {
        ...
        if($ext)
        {
          ...
          temp::assign('link', core::$home.'/admin/statedit?id='.$id);
    ...
    elseif(isset($_POST['del_attachment']))
    {
      ...
      if ($file_del)
      {
        $req_file = core::$db->query("SELECT * FROM `ds_art_files` WHERE `id` = '".$file_del."' AND `post` = '".$id."' LIMIT 1");  
    ...
    elseif(POST('submit'))
    {
       ...
       if(!$error)
       {
         core::$db->query('UPDATE `ds_article` SET `name` = "'.core::$db->res($art_name).'", `text` = "'.core::$db->res($art_text).'", `cache` = "'.core::$db->res(text::presave($art_text)).'" WHERE `id` = "'.$id.'";'); 
         func::notify(lang('pr_adminpanel'), lang('stat_added'), core::$home.'/article?id='.$id, lang('continue'));
       } 
    }  
    ...
    $arr = array();
    $out = array();
    $at1 = core::$db->query('SELECT * FROM `ds_art_files` WHERE `post` = "'.$id.'" ORDER BY `id`;');
    if ($at1->num_rows)
    {
