...
class core
{
  ...
  private static function formid()
  {
    ...
    if($_POST)
    {
      if(!isset($_POST['formid']) or $_POST['formid'] != self::$formid)
      {
  ...
  private static function load_theme()
  {
     ...
     // Определяем тип темы 
     if(!self::$user_set['typetheme'] or !self::$all_themes[self::$user_set['typetheme']])
     {
    ...
    //////////
     ...
     // Определяем тему 
     if(!self::$user_set['theme'][self::$typetheme] or !in_array(self::$user_set['theme'][self::$typetheme], self::$all_themes[self::$typetheme]))
     {
